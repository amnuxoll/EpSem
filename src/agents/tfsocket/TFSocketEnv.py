...
class TFSocketEnv:
    ...
    def __init__(self):
        ...
        self.rolling_avg_steps = 0      # avg_steps but only for the last 10 goals reached
    ...
    def update_rolling_avg_steps(self):
        '''
        Calculate a rolling average of the number of steps to reach the goal for the
        last 10 goals that were reached

        !!!NOTE: NUM_GOALS_TO_AVG must be <= the model creation threshold in TFSocket.py!!!
        '''
        # NOTE: This value was choosen arbitrarily and may perform minorly differently at different FSM environments
        global NUM_GOALS_TO_AVG # TODO: Replace this constant with a dynamic value

        if self.num_goals < NUM_GOALS_TO_AVG:
            return # Not enough history to calculate

        # Extract the last 10 goals worth of steps from the history
